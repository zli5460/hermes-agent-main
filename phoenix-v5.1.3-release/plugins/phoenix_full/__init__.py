...
def _get_phoenix():
    ...
    with _phoenix_lock:
        ...
        try:
            ...
            _phoenix = Phoenix()
            logger.info("[phoenix_full] Phoenix V5.1.3 loaded")
            return _phoenix
...
def _mimo_intent_analyze(text):
    """V5.1: 调MIMO做意图分析，判断任务是否需要深度AI处理。
...
def register(manager):
    ...
    logger.info("[phoenix_full] hooks registered (V5.1.3: manual upgrade + auto fallback)")
...
def _classify_route(user_message):
    """V5.1.3: 自动升档已关闭。保留函数只为旧调用兼容，固定返回 daily。"""
...
def _resolve_provider_for_model(model_name, provider_name=None):
    """V5.1: 用 Phoenix ProviderResolver 解析完整 provider/base_url/api_key。
...
def _do_model_switch(agent, target_model, tier):
    """调用 AIAgent.switch_model() 做完整切换。V5.1.3：primary失败后按本档 fallback/emergency 依次尝试。"""
    tried = []
    try:
...
def _on_pre_api_request(**kwargs):
    ...
    # Phoenix 5.1.3: 关闭 RouterEngine 自动路由。
