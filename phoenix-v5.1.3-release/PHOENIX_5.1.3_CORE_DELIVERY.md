...
- **不死鸟 V5.1.3 做的事**：在**同一条对话管线**里，额外识别 **`/深度`、`/大神`、`/真神`** 三条**业务口令**，走「确认 → 真 `switch_model` → fallback/emergency」——这是 **加一层「档位任务」**，**不是**取消 Hermes 自带的模型切换。
...
**结论**：「连入口都改了」属于 **错误部署或旧文档害人**，不是 V5.1.3 设计目标；交付时应把 **`check_hermes_entrypoint` 必须通过** 当作硬门槛。
...
本仓库 **`sim_verify_v513.py`** 已覆盖：包完整性、敏感信息扫描、install 静态链、`phoenix_open` 手动挡语义、插件静态审计、doctor 隔离环境等。
