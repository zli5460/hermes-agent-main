...
if any(m in blob for m in markers):
    ...
    print("     现象：输入 hermes 出现 '不死鸟 Phoenix V5.1 — CLI入口'，学员会进不了 Hermes。")
...
for f in phoenix.py __init__.py cli.py doctor.py config.json auto_save.py auto_fusion.py post_upgrade_hook.sh install.sh install.ps1 VERSION.md CLAUDE.md README.md START_HERE.md DELIVERY_GUIDE.md INSTALL_GUIDE.md USER_GUIDE.md ARCHITECTURE.md phoenix.yaml; do
  [ -f "$PKG_DIR/$f" ] && cp -f "$PKG_DIR/$f" "$PHOENIX_HOME/$f"
done
for f in \
  "不死鸟_Phoenix_V5.1.3_使用说明书.md" \
  "不死鸟_Phoenix_V5.1.3_技术细则与路径原理.md" \
  INSTALL_安装说明_完整版.md \
  ...
  PHOENIX_5.1.3_CORE_DELIVERY.md \
  Phoenix_V5.1.3_初心回归_架构蓝图.md; do
  [ -f "$PKG_DIR/$f" ] && cp -f "$PKG_DIR/$f" "$PHOENIX_HOME/$f"
done
...
echo "  🧭 写入 Phoenix V5.1.3 初心回归版手动路由配置..."
