...
function Test-HermesEntrypoint([string]$Phase) {
    ...
    $markers = @("不死鸟 Phoenix", "python -m phoenix.cli", "Phoenix V5.1", "CLI入口")
    try {
    ...
    foreach ($m in $markers) {
        if ($blob.Contains($m)) {
            ...
            Write-Host "     现象：输入 hermes 出现 '不死鸟 Phoenix V5.1 — CLI入口'，学员会进不了 Hermes。" -ForegroundColor Yellow
...
if 'inspect.signature(cli_main)' in text and 'Phoenix V5.1.3 compatibility' in text:
...
new = """    # Filter out None values
    ...
    # Phoenix V5.1.3 compatibility: some WSL/student Hermes installs carry an
...
$files = @(
    ...
    "VERSION.md","CLAUDE.md","README.md","START_HERE.md","DELIVERY_GUIDE.md","INSTALL_GUIDE.md","USER_GUIDE.md","ARCHITECTURE.md","phoenix.yaml",
    "不死鸟_Phoenix_V5.1.3_使用说明书.md","不死鸟_Phoenix_V5.1.3_技术细则与路径原理.md",
    "INSTALL_安装说明_完整版.md","INSTALL_平台选择_macOS_Windows_WSL_Linux.md","INSTALL_CUSTOMER_一页纸.md","INSTALL_装前装后对比_Hermes与不死鸟.md",
    "RELEASE_交付流程与验收.md","HERMES_融合与随版本升级.md","HERMES_TUI_与不死鸟斜杠指令.md","PHOENIX_5.1.3_CORE_DELIVERY.md","Phoenix_V5.1.3_初心回归_架构蓝图.md"
)
foreach ($f in $files) {
...
Write-Host "  🧭 写入 Phoenix V5.1.3 手动升档路由配置..." -ForegroundColor Cyan
$configPy = @'
from pathlib import Path
import json, sys
...
Write-Host ""
Write-Host "🔥 Phoenix V5.1.3 路由启动方式：" -ForegroundColor Cyan
