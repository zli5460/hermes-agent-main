...
**完整安装说明（含每一步问答、Key 配置、交互说明）**：请优先阅读 **[INSTALL_安装说明_完整版.md](./INSTALL_安装说明_完整版.md)**。
...
## 安装步骤
macOS/Linux：`bash install.sh`。Windows：使用 PowerShell 安装脚本（如包内提供）。安装器只写客户本机配置，不会输出明文 Key。
